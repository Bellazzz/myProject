...
$sql = "SELECT COUNT( * ) AS allRecord
		FROM products
		WHERE prdtyp_id 
		NOT IN ('PT12',  'PT02',  'PT03',  'PT04',  'PT05', 'PT01')";
...
if($page <= 1) {
	...
	$prevPage = $page-1;
	$prevPageLink = "shop_foods.php?sort=$sort&page=$prevPage&itemDisplay=$itemDisplay";
}
if($page >= $allPage) {
	$nextPageLink = '#fakelink';
} else {
	$nextPage = $page+1;
	$nextPageLink = "shop_foods.php?sort=$sort&page=$nextPage&itemDisplay=$itemDisplay";
}
