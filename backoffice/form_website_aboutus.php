...
if(!$_REQUEST['ajaxCall']) {
	...
	if($action == 'ADD') {
		...
		// Rename Image
		if(strpos($formData['wsa_picture'], 'temp_') !== FALSE) {
			$type		= str_replace(".", "", strrchr($formData['wsa_picture'],"."));
			$tmpRecord	= new TableSpa('website_aboutus', null);
			$wsa_picture	= $tmpRecord->genKeyCharRunning().".$type";
			$wsa_picture_path = '../img/website_aboutus/'.$wsa_picture;

			// Delete Old Image
			if(file_exists($wsa_picture_path)) {
				if(!unlink($wsa_picture_path)) {
			...
			if(rename('../img/temp/'.$formData['wsa_picture'], $wsa_picture_path)) {
				$formData['wsa_picture'] = $wsa_picture;
			} else {
		...
		if(strpos($formData['wsa_picture'], 'temp_') !== FALSE) {
			$type		= str_replace(".", "", strrchr($formData['wsa_picture'],"."));
			$wsa_picture	= $code.".$type";
			$imgTmpPath = '../img/temp/'.$formData['wsa_picture'];
			$imgNewPath = '../img/website_aboutus/'.$wsa_picture;
