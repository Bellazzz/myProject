...
if(!$_REQUEST['ajaxCall']) {
	...
	if($action == 'ADD') {
		...
		// Rename Image
		if(strpos($formData['elechk_pic'], 'temp_') !== FALSE) {
			$type		= str_replace(".", "", strrchr($formData['elechk_pic'],"."));
			$tmpRecord	= new TableSpa('element_checks', null);
			$elechk_pic	= $tmpRecord->genKeyCharRunning().".$type";
			$elechk_pic_path = '../img/element_checks/'.$elechk_pic;

			// Delete Old Image
			if(file_exists($elechk_pic_path)) {
				if(!unlink($elechk_pic_path)) {
			...
			if(rename('../img/temp/'.$formData['elechk_pic'], $elechk_pic_path)) {
				$formData['elechk_pic'] = $elechk_pic;
			} else {
		...
		if(strpos($formData['elechk_pic'], 'temp_') !== FALSE) {
			$type		= str_replace(".", "", strrchr($formData['elechk_pic'],"."));
			$elechk_pic	= $code.".$type";
			$imgTmpPath = '../img/temp/'.$formData['elechk_pic'];
			$imgNewPath = '../img/element_checks/'.$elechk_pic;
